...
/**
 * Scrub list for the migrate `ms` command telemetry. `Migrate.MsCommand.Start`/`.Complete` log the
 * exact `ms` command line, which for `ms app init` includes the user-supplied `--display-name` and
 * `--description`. Those can contain emails or secrets, so scrub the event if the command line
 * matches any known email/key pattern.
 */
const MigrateCommandScrubConfigList = [
    EmailScrubConfig,
    GoogleApiKeyScrubConfig,
    SasKeySigScrubConfig,
    SymmKey256ScrubConfig,
];
/**
...
export const EventsToScrubConfig = {
    ...
    'PowerAppsClient.Activity': {
        operations: {
            ...
            'Migrate.MsCommand.Start': MigrateCommandScrubConfigList,
            'Migrate.MsCommand.Complete': MigrateCommandScrubConfigList,
        },
    },
