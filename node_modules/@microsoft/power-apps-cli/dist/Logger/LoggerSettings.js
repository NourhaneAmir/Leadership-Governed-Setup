...
import { getCliEnvVar, getSystemEnvVar } from '../Utils/EnvVars.js';
/**
...
function isPacCliTelemetryEnabled() {
    if (isTruthy(getSystemEnvVar('PAC_CLI_TELEMETRY_OPTOUT'))) {
        return false;
    }
    const localAppData = getSystemEnvVar('LOCALAPPDATA');
    if (!localAppData) {
        return undefined;
    }
 ...
 *    - PA_CLI_TELEMETRY=0/false/no/off forces `enabled: false`
...
export function resolveUserTelemetrySettings() {
    ...
    const cliTelemetry = getCliEnvVar('TELEMETRY');
    if (cliTelemetry !== undefined && isFalsy(cliTelemetry)) {
