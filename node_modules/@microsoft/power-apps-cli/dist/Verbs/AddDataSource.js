...
export async function addDataSourceAsync(argumentProvider) {
    ...
    if (argumentProvider.isJsonMode() && argumentProvider.isGrouped()) {
        const connector = await argumentProvider.getOption('apiId', true);
        const table = await argumentProvider.getOption('resourceName', true);
        emitJson(argumentProvider, { connector, ...(table ? { table } : {}) });
        return;
    }
    ...
    const push = surfaceCommand('pa app push', 'npx power-apps push', grouped);
    printInfo(`Hint: Run '${run}' to test locally, or '${push}' to deploy.`);
}
function createOptionSchema(argumentProvider) {
    ...
    argumentProvider.addExamples(HelpStrings.Verbs['Add-data-source']);
