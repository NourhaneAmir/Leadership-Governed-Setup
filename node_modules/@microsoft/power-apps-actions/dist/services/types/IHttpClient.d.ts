...
export type RequestConfig = {
    ...
    /** AbortSignal forwarded to the underlying `fetch` — e.g. `AbortSignal.timeout(ms)` */
    signal?: AbortSignal;
    /** The type of response for the request. Defaults to json */
    responseType?: 'json' | 'text' | 'blob';
    /**
     * The body of the request to send. A plain object is JSON-serialized; a string is sent
     * verbatim (when Content-Type is not JSON); binary payloads (`Uint8Array`/`Buffer`/`ArrayBuffer`)
     * are sent as raw bytes unchanged — use these for binary assets so they are not corrupted.
     */
    body?: unknown;
