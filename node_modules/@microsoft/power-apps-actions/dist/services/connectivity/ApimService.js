...
/**
 * Returns the baseUrl of the highest-numbered CDPTabular1 revision, preferring Production over
 * Preview. Returns null if no CDPTabular1 interface exists or it has no revisions.
 */
function getLatestCdpTabularBaseUrl(connector) {
    ...
    const revisions = Object.entries(cdpTabular1.revisions)
        .map(([version, rev]) => ({ version: parseInt(version, 10), rev }))
        .sort((a, b) => b.version - a.version);
    const production = revisions.find((r) => r.rev.status === 'Production');
    if (production) {
        return production.rev.baseUrl;
    }
    const preview = revisions.find((r) => r.rev.status === 'Preview');
    return preview ? preview.rev.baseUrl : null;
}
function getRuntimeRequestUrl(connector, connection) {
    const baseUrl = getLatestCdpTabularBaseUrl(connector);
    if (baseUrl === null) {
        throw new Error("No usable revisions found for the 'CDPTabular1' interface in the connector's interfaces.");
    }
 ...
 *
 * The connection PUT/GET response may include `properties.testLinks`, an array
 * of `{ requestUri, method }` entries owned by the API Hub runtime. When present,
 * we invoke the first entry, using the API Hub bearer audience for the current
 * region. `testLinks` is optional — when it is absent we skip verification and
 * trust the PUT.
 */
export async function testConnectionAsync(connection, logger) {
    ...
    try {
        ...
        if (!testLink?.requestUri) {
            // No test link — connectors are not required to provide one. Trust the
            // PUT and skip verification (Canvas: `if (!connection.testLink) return;`).
            additionalScenarioData = { skipped: true, reason: 'no-test-link' };
            scenario?.complete(additionalScenarioData);
            return;
        }
        const { httpClient, region } = getPlayerServiceConfig();
