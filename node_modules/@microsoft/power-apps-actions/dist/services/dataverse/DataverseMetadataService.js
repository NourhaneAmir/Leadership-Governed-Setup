...
import { parseCsdl } from './CsdlMetadataParser.js';
export class DataverseMetadataService {
    ...
    async getEntityDefinitionAsync(orgUrl, tableName, includeOptionSets = true, logger) {
        ...
        try {
            ...
            errorMessage +=
                `\nVerify the table's logical name is correct and that it exists in this environment` +
                    ` (a 404 usually means the table name is misspelled), and that your account has permission` +
    ...
    async searchCsdlOperations(orgUrl, searchTerm, logger) {
        const { operations } = await this.getCsdlMetadataAsync(orgUrl, searchTerm, logger);
        return operations;
    }
    /**
     * Fetches and parses the OData $metadata CSDL document once, returning both the Action/Function
     * definitions (filtered by `searchTerm`) and the registry of concrete entity types. Callers that
     * need to resolve entity references (e.g. to skip abstract/non-entity `mscrm.*` parameter types)
     * should use this instead of {@link searchCsdlOperations} to avoid a second $metadata download.
     */
    async getCsdlMetadataAsync(orgUrl, searchTerm, logger) {
        ...
        try {
            ...
            const metadata = parseCsdl(response.data, searchTerm);
            additionalScenarioData.resultCount = metadata.operations.length;
            additionalScenarioData.entityTypeCount = metadata.concreteEntityTypes.size;
            scenario.complete(additionalScenarioData);
            return metadata;
        }
        catch (error) {
