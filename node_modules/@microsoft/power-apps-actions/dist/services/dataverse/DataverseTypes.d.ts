...
/**
 * Body shape for the Dataverse `AddSolutionComponent` action used to link an
 * already-solution-aware app (or other component) into an additional solution.
 * Component types follow the Dataverse `solutioncomponent.componenttype` enum;
 * canvas apps and code apps share component type 300.
 */
...
/**
 * The result of parsing a Dataverse OData $metadata CSDL document: the operations plus a registry
 * of which `mscrm.*` types are concrete, queryable entity tables.
 *
 * A type referenced by an operation parameter (e.g. `mscrm.account`) only has an `EntityDefinitions`
 * metadata entry when it is declared as a non-abstract `<EntityType>` in the CSDL. Abstract base
 * types (`<EntityType ... Abstract="true">`, e.g. `crmbaseentity`) and non-entity types
 * (`<EnumType>` / `<ComplexType>`, e.g. `EndpointAccessType`) are NOT queryable and must be skipped
 * before attempting a metadata fetch — otherwise the fetch returns HTTP 404.
 */
export interface CsdlMetadata {
    /** All <Function> and <Action> operations, optionally filtered by name. */
    operations: CsdlOperation[];
    /**
     * Logical names of all concrete (non-abstract) `<EntityType>` elements declared in the document,
     * e.g. `"account"`, `"opportunity"`. Excludes abstract base types and non-entity types.
     */
    concreteEntityTypes: Set<string>;
}
