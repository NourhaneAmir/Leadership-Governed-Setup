...
// Mirrors RP's `DataSourceShareLink` contract. RP tolerates null shareId/url (C# `string?`),
// but this schema validates what we *produce*: the CLI always supplies concrete linkKind/shareId/url,
// so they are required here. listId/siteId are genuinely optional (only some share links carry them).
export const ShareLinkDetailsSchema = z.object({
    linkKind: z.number(),
    shareId: z.string(),
    url: z.string(),
    // Present for SharePoint list share links; optional because not every share link is list-scoped.
    listId: z.string().optional(),
    // Present for SharePoint-based share links; optional because not every share link is site-scoped.
...
export const ConnectionReferenceSchema = z.object({
    ...
    isOnPremiseConnection: z.boolean().optional(),
    gatewayObjectIdHint: z.string().optional(),
});
