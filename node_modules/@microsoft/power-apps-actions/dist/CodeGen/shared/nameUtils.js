...
/** Returns a stable 8-character hex hash of the input string (djb2 variant). */
...
export function sanitizeName(name, removeWhitespace = true, prefixReservedKeywords = true) {
    ...
    // Capture the normalized form (after trim + optional whitespace removal) so that the
    // hash fallback is stable: e.g. '客户订单' and '客户订单 ' normalize identically and
    // must produce the same hash identifier.
    ...
    // When all characters were non-ASCII (e.g. Chinese, Arabic, Cyrillic) or all
    // punctuation, the result is all underscores. Fall back to a stable hash-based
    // identifier so add-data-source doesn't crash on such names.
    if (name.length === 0 || /^_+$/.test(name)) {
        name = `_${stableHash(normalizedName)}`;
    }
...
export function getSanitizedUniqueName(existingNames, desiredName) {
    // Sanitize the base name. Comparison is case-insensitive: the returned alias is always
    // lowercased, and existing aliases (e.g. those persisted in power.config.json) are lowercased
    // too, so a case-preserved desiredName — such as a tabular friendlyName like `Orders` — must
    // still collide with an existing `orders` and get disambiguated.
    const base = sanitizeName(desiredName).toLowerCase();
    const nameSet = new Set(existingNames.map((name) => name.toLowerCase()));
    if (!nameSet.has(base)) {
        return base;
    }
    let maxSuffix = 0;
