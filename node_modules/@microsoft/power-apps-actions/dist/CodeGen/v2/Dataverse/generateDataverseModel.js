...
function addOptionsetDeclarations(sourceFile, modelName, propertiesNode) {
    for (const [propName, propValue] of Object.entries(propertiesNode)) {
        ...
        // The display label is emitted as a runtime string value (the type alias below is
        // `keyof typeof`, i.e. the numeric keys), so it must preserve the original localized
        // text (Vietnamese, CJK, etc.) rather than being collapsed to an identifier.
        const entries = enumLabels.map((label, i) => {
            const labelLiteral = !label || (typeof label === 'string' && label.trim() === '')
                ? `'EmptyOption${i}'`
                : formatEnumLabel(label);
            return `  ${enumValues[i]}: ${labelLiteral}`;
        });
