...
import { formatEnumLabel, generateOptionsetEnumName, getDataverseMultiSelectPicklistFields, getDataverseOwnerColumnNames, hasOptionsetData, isDataverseEntity, isDataverseSystemManagedProperty, } from './shared/dataverseUtils.js';
...
function generateDataverseBaseInterface(interfaceName, originalModelName, definition, lookupColumns) {
    ...
    for (const [propName, propValue] of Object.entries(propertiesNode)) {
        ...
        // Skip system-managed properties (e.g. ownerid, owneridtype, owneridname). Dataverse
        // owns these values and rejects create/update requests that attempt to set them.
        if (propValueAsObject &&
            isDataverseSystemManagedProperty(propName, propValueAsObject, ownerColumnNames)) {
            continue;
        }
...
function generateDataverseExtendedInterface(modelName, baseInterfaceName, definition, lookupColumns) {
    ...
    for (const [propName, propValue] of Object.entries(propertiesNode)) {
        const propValueAsObject = isJsonObject(propValue) ? propValue : undefined;
        // Include read-only and system-managed properties in the extended (read) interface.
        // System-managed properties (e.g. ownerid, owneridtype, owneridname) are readable but
        // not writable, so they belong here rather than in the base (write) interface.
        const isReadOnly = !!propValueAsObject?.['x-ms-read-only'];
        const isSystemManaged = !!propValueAsObject &&
            isDataverseSystemManagedProperty(propName, propValueAsObject, ownerColumnNames);
        if (!isReadOnly && !isSystemManaged) {
            continue;
        }
    ...
    if (enumValues && Array.isArray(enumValues) && enumLabels && Array.isArray(enumLabels)) {
        ...
        if (hasObjectFormat) {
            ...
            for (let i = 0; i < enumLabels.length; i++) {
                ...
                // Emit the display name directly as the runtime string value. The type alias below
                // is `keyof typeof`, i.e. the numeric keys, so the value must preserve the original
                // localized label (Vietnamese, CJK, etc.) instead of being collapsed to an
                // identifier. Empty labels get a generated placeholder.
                const labelLiteral = !displayName || (typeof displayName === 'string' && displayName.trim() === '')
                    ? `'EmptyOption${i}'`
                    : formatEnumLabel(displayName);
                const comma = i < enumLabels.length - 1 ? ',' : '';
                lines.push(`${padding}  ${numericValue}: ${labelLiteral}${comma}`);
            }
 ...
 * entity reference (`mscrm.*`) or entity collection (`Collection(mscrm.*)`). Entity collections
 * without a generated model fall back to `Record<string, unknown>[]`. Returns `null` when the
 * field is absent, not an entity reference, or a non-collection entity's logical name is not
 * present in `entityModelMap` (meaning no model file can be guaranteed).
 *
...
function addCommonModelCode(lines) {
    ...
    lines.push('  /**');
    lines.push('   * When true, requests the server-side total count of records matching the query,');
    lines.push('   * returned as `count` on the result. The total is independent of `top`/`skip`/paging');
    lines.push('   * (it reflects the full result set, not the number of rows loaded).');
    lines.push('   * Note: Dataverse caps this value at 5000 — larger result sets report 5000.');
    lines.push('   */');
...
function generateDataverseCreateMethod(modelName, primaryKey, hasLookupColumns, multiSelectPicklistFields) {
    ...
    return hasMultiSelect
        ? [
            ...
            `      ${getServiceName(modelName)},`,
            `      serializeMultiSelectPicklistFields(record as unknown as Record<string, unknown>, ${modelName}Service.multiSelectPicklistFields)`,
            `    );`,
            `    if (result.data) {`,
            `      deserializeMultiSelectPicklistFields(result.data as unknown as Record<string, unknown>, ${modelName}Service.multiSelectPicklistFields);`,
            `    }`,
            `    return result;`,
            `  }`,
        ]
...
function generateDataverseUpdateMethod(modelName, primaryKey, hasLookupColumns, multiSelectPicklistFields) {
    ...
    return hasMultiSelect
        ? [
            ...
            `      ${getServiceName(modelName)},`,
            `      id,`,
            `      serializeMultiSelectPicklistFields(changedFields as unknown as Record<string, unknown>, ${modelName}Service.multiSelectPicklistFields)`,
            `    );`,
            `    if (result.data) {`,
            `      deserializeMultiSelectPicklistFields(result.data as unknown as Record<string, unknown>, ${modelName}Service.multiSelectPicklistFields);`,
            `    }`,
...
function generateDataverseGetAllMethod(modelName, multiSelectPicklistFields) {
    ...
    return hasMultiSelect
        ? [
            ...
            `      options`,
            `    );`,
            `    result.data?.forEach(record => deserializeMultiSelectPicklistFields(record as unknown as Record<string, unknown>, ${modelName}Service.multiSelectPicklistFields));`,
            `    return result;`,
            `  }`,
        ]
        : [
