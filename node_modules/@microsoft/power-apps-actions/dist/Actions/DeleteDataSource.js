...
export async function deleteDataSourceAsync(config) {
    ...
    if (!sqlStoredProcedure) {
        ...
        // For connector data sources, `add-data-source` defaults the data source name to the
        // connector's api name (see AddDataSource's `desiredName = apiName`). When the caller omits
        // the name, fall back to the apiId so `delete-data-source -a <connector>` works for the common
        // single-data-source-per-connector case without redundantly repeating the connector name via
        // -n. SQL (stored-procedure / table named) and Dataverse (table named) are excluded because
        // their data sources are never named after the connector, so they still require an explicit
        // name. Callers targeting a disambiguated or tabular data source also pass -n explicitly.
        if (!dataSourceName && apiId !== 'sql' && apiId !== DataSources.Dataverse.toLowerCase()) {
            dataSourceName = apiId;
        }
    }
