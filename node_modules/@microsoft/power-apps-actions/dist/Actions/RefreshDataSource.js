...
import { getConnectionByNameAsync, getConnectorAsync, getFlowMetadataAsync, listConnectionsForConnectorAsync, tryGetGatewayObjectId, } from '../services/index.js';
...
async function findConnectionIdForRefAsync(connectorFullId, authenticationType, logger) {
    ...
    // Detect on-prem connections using the same source of truth as AddDataSource:
    // tryGetGatewayObjectId inspects the connection value for the connector parameter
    // that is typed `gatewaysetting`. Fetch the connector definition so we can apply it.
    // If the connector can't be fetched, fall back to the looser value-scan below.
    const connector = await getConnectorAsync(connectorId, logger);
    const isOnPremConnection = (conn) => connector !== undefined && connector !== null
        ? tryGetGatewayObjectId(conn, connector) !== undefined
        : hasGatewayParameterValue(conn);
    const match = connections.find((conn) => {
        // Skip on-premises gateway connections; we have no stored gateway info to match against.
        if (isOnPremConnection(conn)) {
            return false;
        }
...
/**
 * Fallback on-prem detection used only when the connector definition is unavailable and
 * `tryGetGatewayObjectId` can't be applied. Flags a connection as on-prem when any of its
 * parameter values carries a non-empty `gatewayObjectId`. This is intentionally looser than
 * the connector-typed check, but errs toward excluding gateway connections from the cloud
 * fallback match, which is the desired safety behavior here.
 */
function hasGatewayParameterValue(conn) {
    const params = conn.properties?.connectionParameters;
    if (!params) {
        return false;
    }
    for (const value of Object.values(params)) {
        if (value !== null &&
            typeof value === 'object' &&
            typeof value.gatewayObjectId === 'string' &&
            value.gatewayObjectId !== '') {
            return true;
        }
    }
    return false;
