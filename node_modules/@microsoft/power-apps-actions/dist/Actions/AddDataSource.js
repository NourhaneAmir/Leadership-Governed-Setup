...
import { isJsonObject } from '../CodeGen/shared/schemaUtils.js';
...
import { DataverseMetadataService, DataverseSystemDataService, getConnectionByNameAsync, getConnectorAsync, getPlayerServiceConfig, getSqlStoredProcedureMetadataAsync, getTableMetadataAsync, getTabularConnectorVersion, isConnectionShareable, tryGetGatewayObjectId, } from '../services/index.js';
...
async function addCdpDataSourceAsync(config, apiId) {
    ...
    if (isEnvironmentVariable(dataset)) {
        ...
        // Assign the raw resolved value; ensureEncoded (in ApimService) is the single
        // source of URL encoding: it splits the dataset on commas and encodes each part,
        // keeping the comma as a literal separator. Pre-encoding here would turn the comma
        // into %252C, so ensureEncoded can no longer split on it and the APIM metadata URL
        // ends up with an encoded comma, which can produce a 404.
        if (resolvedEnvVariable) {
            dataset = resolvedEnvVariable;
        }
    }
    ...
    const gatewayObjectIdHint = tryGetGatewayObjectId(connection, connector);
    let metadata;
...
async function handleAddCdpDataSourceAsync(params, logger, skipCodeGen) {
    const { metadata, connector, schemaPath, codeGenPath, configPath, tabularMetadata, authenticationType, sharedConnectionId, connectionReferenceLogicalName, siteId, gatewayObjectIdHint, } = params;
    ...
    const { dataSourceName, spEntity, modelNameSuffix, preserveExistingModelNameSuffix } = await updateConnectionReferences(configPath, {
        apiId,
        apiName,
    ...
    const schemaFilePath = await saveMetadata(metadata, apiName, dataSourceName, schemaPath, spEntity, logger, connectorVersion, modelNameSuffix, preserveExistingModelNameSuffix);
    await processDataSourceInfo(schemaPath, logger);
    if (!skipCodeGen) {
        await generateModelService({
            schemaFolderPath: schemaPath,
 ...
 */
async function saveMetadata(metadata, apiName, dsName, schemaPath, spEntity, logger, connectorVersion, modelNameSuffix, preserveExistingModelNameSuffix) {
    ...
    try {
        ...
        let annotatedMetadata = metadata;
        if (connectorVersion !== undefined) {
            if (!isJsonObject(metadata)) {
                throw new Error(`Tabular metadata for '${dsName}' must be a JSON object.`);
            }
            annotatedMetadata = {
                ...metadata,
                'x-ms-connection-version': connectorVersion,
                ...(!spEntity && {
                    'x-ms-model-name-suffix': preserveExistingModelNameSuffix
                        ? await inferExistingModelNameSuffix(filePath, dsName, modelNameSuffix ?? '', logger)
                        : (modelNameSuffix ?? ''),
                }),
            };
        }
...
async function inferExistingModelNameSuffix(filePath, dsName, fallbackSuffix, logger) {
    const vfs = getVfs();
    if (!(await vfs.exists(filePath))) {
        return fallbackSuffix;
    }
    const existingContents = await vfs.readFile(filePath, 'utf-8');
    let existingMetadata;
    try {
        existingMetadata = JSON.parse(existingContents);
    }
    catch (error) {
        if (!(error instanceof SyntaxError)) {
            throw error;
        }
        trackInfoEvent(logger, 'SaveMetadata.InvalidExistingSchema', {
            filePath,
            reason: 'MalformedJson',
        });
        return fallbackSuffix;
    }
    if (!isJsonObject(existingMetadata)) {
        trackInfoEvent(logger, 'SaveMetadata.InvalidExistingSchema', {
            filePath,
            reason: 'NonObjectJson',
        });
        return fallbackSuffix;
    }
    const annotation = existingMetadata['x-ms-model-name-suffix'];
    if (isValidModelNameSuffix(annotation)) {
        return annotation;
    }
    const existingName = typeof existingMetadata.title === 'string' && existingMetadata.title.trim()
        ? existingMetadata.title
        : existingMetadata.name;
    if (typeof existingName !== 'string' || !existingName.trim()) {
        return fallbackSuffix;
    }
    return (tryDeriveModelNameSuffix(dsName, sanitizeTabularResourceName(existingName).toLowerCase()) ??
        fallbackSuffix);
}
function isValidModelNameSuffix(value) {
    return typeof value === 'string' && (value === '' || /^_\d+$/.test(value));
}
function tryDeriveModelNameSuffix(dataSourceName, aliasBase) {
    if (dataSourceName === aliasBase) {
        return '';
    }
    const prefix = `${aliasBase}_`;
    const remainder = dataSourceName.startsWith(prefix)
        ? dataSourceName.slice(prefix.length)
        : undefined;
    return remainder && /^\d+$/.test(remainder) ? `_${remainder}` : undefined;
}
/**
...
async function updateConnectionReferences(configPath, params, logger) {
    ...
    try {
        ...
        const { dataSourceName, spEntity, modelNameSuffix, preserveExistingModelNameSuffix } = upsertConnectionReference(config.connectionReferences, {
            ...params,
        ...
        return {
            ...
            spEntity,
            modelNameSuffix,
            preserveExistingModelNameSuffix,
        };
    }
    catch (error) {
