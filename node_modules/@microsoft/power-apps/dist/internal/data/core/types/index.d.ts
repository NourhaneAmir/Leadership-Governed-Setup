...
export interface IOperationResult<TResponse> {
    ...
    skipToken?: string;
    /**
     * Total number of records matching the query, populated when the request sets
     * `count: true`. This is the server-side total for the full result set — it is
     * independent of `top`, `skip`, and paging, so it is not the number of records
     * returned in `data`.
     *
     * Note: For Dataverse this comes from the `@odata.count` annotation, which the
     * platform caps at 5000; result sets larger than 5000 report 5000.
     * @type {number}
     */
    count?: number;
    /**
