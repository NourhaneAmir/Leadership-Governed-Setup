...
export class ConnectorDataOperationExecutor {
    ...
    async executeAsync(operation) {
        try {
            ...
            // A binary body (decoded from a base64 `format: binary` parameter) must be sent as
            // application/octet-stream so the runtime client wraps it in a binary Blob. Without this
            // header the bytes are wrapped in an application/json Blob and the gateway forwards the
            // ASCII content verbatim, corrupting uploaded files.
    ...
    async _buildOperationBodyParam(operation, tableName) {
        ...
        if (bodyParam) {
            ...
            if (value !== undefined && value !== null) {
                // For binary parameters, the value is a base64 string that must be sent as raw bytes.
                // Decode it to a Uint8Array so executeAsync can flag the request as
                // application/octet-stream. Returning the base64 string (or JSON.stringify'ing it) causes
                // the gateway to forward ASCII content verbatim, corrupting uploads.
